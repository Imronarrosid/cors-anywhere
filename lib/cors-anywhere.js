...
import { createServer as _createServer } from 'http-proxy';
import { isIPv4, isIPv6 } from 'net';
import { resolve, parse } from 'url';
import { test } from './regexp-top-level-domain';
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  if (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308) {
    ...
    var parsedLocation;
    if (locationHeader) {
      locationHeader = resolve(requestState.location.href, locationHeader);
...
export function createServer(options) {
  ...
  var proxy = _createServer(httpProxyOptions);
